...
class resnet(nn.Module):
    def __init__(self, block, layers, num_classes=1, theta_dim=3):
        ...
        self.conv_1 = nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn_1 = nn.BatchNorm2d(32)
        self.relu = nn.ReLU(inplace=True)

        self.layer_1 = self._make_layer(block, 32, layers[0], stride=1)
        self.layer_2 = self._make_layer(block, 64, layers[1], stride=2)
        self.layer_3 = self._make_layer(block, 128, layers[2], stride=2)
        self.layer_4 = self._make_layer(block, 256, layers[3], stride=2)

        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc_1 = nn.Linear(256 * block.expansion + theta_dim, 128, bias=True)
        self.fc_2 = nn.Linear(128, 64, bias=True)
        self.fc_3 = nn.Linear(64, 32, bias=True)
        self.fc_4 = nn.Linear(32, num_classes, bias=True)
