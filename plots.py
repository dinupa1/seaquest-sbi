...
for i in range(5):
    ...
    plt.hist(posterior[i, :, 0], bins=bins, histtype="step", label=fr"$\lambda_{{fit}}$ = {mean[0]:.3f} +/- {error[0]:.3f}")
    ...
    plt.figure(figsize=(8., 8.))
    plt.hist(posterior[i, :, 1], bins=bins, histtype="step", label=fr"$\lambda_{{fit}}$ = {mean[1]:.3f} +/- {error[1]:.3f}")
    plt.axvline(x=theta[i, 1], linestyle="--", color="r", label=fr"$\lambda_{{true}}$ = {theta[i, 1]:.3f}")
    plt.xlabel(r"$\mu$")
    plt.ylabel(r"$p(\mu|x)$")
    plt.legend(frameon=False)
    plt.tight_layout()
    ...
    plt.figure(figsize=(8., 8.))
    plt.hist(posterior[i, :, 2], bins=bins, histtype="step", label=fr"$\lambda_{{fit}}$ = {mean[2]:.3f} +/- {error[2]:.3f}")
    plt.axvline(x=theta[i, 2], linestyle="--", color="r", label=fr"$\lambda_{{true}}$ = {theta[i, 2]:.3f}")
    plt.xlabel(r"$\nu$")
    plt.ylabel(r"$p(\nu|x)$")
    plt.legend(frameon=False)
    plt.tight_layout()
...
score_mean = np.mean(posterior[i, :, :], axis=0)
score_error = np.std(posterior[i, :, :], axis=0)
